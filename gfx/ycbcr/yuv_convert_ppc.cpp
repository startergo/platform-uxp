...
void FilterRows_VMX(uint8_t* ybuf, const uint8_t* y0_ptr,
  ...
  // Compute a weighted average. Source rows may be unaligned.
  do {
    c0 = vec_perm(vec_ld(0, y0_ptr), vec_ld(15, y0_ptr),
                  vec_lvsl(0, y0_ptr));
    c1 = vec_perm(vec_ld(0, y1_ptr), vec_ld(15, y1_ptr),
                  vec_lvsl(0, y1_ptr));

    // Expand to short, since vec_mladd does not exist for char (damn).
    y0 = (vector unsigned short)
      vec_mergeh((vector unsigned char)vector_zero, c0);
    y1 = (vector unsigned short)
      vec_mergeh((vector unsigned char)vector_zero, c1);
    y2 = (vector unsigned short)
      vec_mergel((vector unsigned char)vector_zero, c0);
    y3 = (vector unsigned short)
      vec_mergel((vector unsigned char)vector_zero, c1);

    // Interleave the multiply-add operations.
